...
<style>
    ...
    /* ============================================================
       CUSTOM ROOM SELECT & FACILITY DROPDOWN
       ============================================================ */
    .room-select-wrapper, .facility-select-wrapper {
        position: relative;
    }
    .room-select-trigger {
        width: 100%;
    ...
    .room-select-trigger.is-invalid {
    ...
    .room-select-panel.open, .facility-select-panel.open {
        display: block;
    }
    .room-select-option {
        display: flex;
                            ...
                            <div class="datetime-wrapper">
                                ...
                                <div class="time-input">
                                    ...
                                    <input type="text" name="end_time" id="end_time"
                                           ...
                                           value="{{ old('end_time', \Carbon\Carbon::now()->addHours(1)->format('H:i')) }}"
                                           placeholder="--:--" readonly style="cursor:pointer;background:var(--bg-input);"
...
<script>
    $(document).ready(function() {
        ...
        window.pickRoom = function(id, name, capacity) {
            ...
            $('#roomSelectName').text(name).css('color', '');
            $('#roomSelectCapacity').text('Kapasitas ' + capacity + ' orang').show();

            const photoImg = @json($rooms->mapWithKeys(fn($r) => [$r->id => $r->photos->first()->photo_url ?? '']));
            const url = photoImg[id];
            if (url) {
                $('#roomSelectImg').attr('src', url);
        ...
        // ============================================================
        // CUSTOM FACILITIES CHECKBOX DROPDOWN
        // ============================================================
        ...
        // Pre-populate triggers if old() values exist (e.g. after validation error)
        const preselectedRoomId = $('#room_id').val();
        if (preselectedRoomId) {
            const roomsData = @json($rooms->keyBy('id'));
            const r = roomsData[preselectedRoomId];
            if (r) {
                pickRoom(r.id, r.name, r.capacity);
            }
        }
