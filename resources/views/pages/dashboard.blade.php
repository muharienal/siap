...
<style>
    ...
    .schedule-legend .dot.priority  { background: #3b82f6; }
    .schedule-legend .dot.priority-pending { background: #8b5cf6; }
    ...
    /* Booking tipe Priority: biru kalau sudah disetujui, ungu kalau masih pending */
    .cal-event.cal-event-priority-approved {
        background: rgba(59,130,246,0.10) !important;
        border-left-color: #3b82f6 !important;
        z-index: 3;
    }
    .cal-event.cal-event-priority-approved .cal-event-title,
    .cal-event.cal-event-priority-approved .cal-event-status { color: #1d4ed8 !important; }

    .cal-event.cal-event-priority-pending {
        background: rgba(139,92,246,0.10) !important;
        border-left-color: #8b5cf6 !important;
        z-index: 3;
    }
    .cal-event.cal-event-priority-pending .cal-event-title,
    .cal-event.cal-event-priority-pending .cal-event-status { color: #6d28d9 !important; }

    /* Booking Regular yang otomatis ketolak karena kalah sama Priority: diburemin */
    .cal-event.cal-event-bumped {
        filter: blur(1.5px);
        opacity: 0.55;
        z-index: 1;
        transition: filter var(--transition-fast), opacity var(--transition-fast), transform var(--transition-fast);
    }
    .cal-event.cal-event-bumped:hover {
        filter: none;
        opacity: 1;
        transform: translateY(-1px);
    }
...
<div class="dashboard-content">
    ...
    <div class="schedule-card">
                                ...
                                @else
                                    ...
                                    @foreach($dayBookings->where('room_id', $room->id) as $booking)
                                        @php
                                            ...
                                            $isPriority = $booking->booking_type === 'Priority';
                                            $isBumpedByPriority = $booking->status == 2
                                                && $booking->booking_type === 'Regular'
                                                && str_contains((string) $booking->rejection_reason, 'peminjaman prioritas');

                                            $eventClasses = 'cal-event';
                                            if ($isPriority && $booking->status == 1) {
                                                $eventClasses .= ' cal-event-priority-approved';
                                            } elseif ($isPriority && $booking->status == 0) {
                                                $eventClasses .= ' cal-event-priority-pending';
                                            }
                                            if ($isBumpedByPriority) $eventClasses .= ' cal-event-bumped';
                                        @endphp
                                        <div class="{{ $eventClasses }}" role="button" tabindex="0"
        ...
        <div class="schedule-legend">
            ...
            <span class="legend-item"><span class="dot priority"></span> Priority Disetujui</span>
            <span class="legend-item"><span class="dot priority-pending"></span> Priority Pending</span>
            @if($isToday && !$isWeekend)
